...
int Decoder::Generate() {
	...
	for (int i = 0; i < this->file_path.size(); i++) {
		...
		for (int i = 0; i < note_list.size(); i++) {
			if (note_list[i].IsBackup()) {
				auto backup_xml = GetBackupXML(&doc_template, attribute.GetDivision() * measure * 4.0);
			...
			if (i != note_list.size() - 1 && !note_list[i + 1].GetChord() && !note_list[i + 1].IsBackup()) {
